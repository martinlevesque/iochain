...
Instead, just log it to the console.`,
  ...
  fn: async function(inputs, exits) {
    ...
    sails.log.debug('Mail environment: ', sails.config.environment);
    sails.log.debug('Mail isToAddressConsideredFake: ', isToAddressConsideredFake);
...
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-`);
      ...
      await mailer.send(inputs.to, inputs.subject, htmlEmailContents);
    }
