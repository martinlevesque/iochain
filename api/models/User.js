...
module.exports = _.merge(Base('User'), {
...
email status until they click the link in the confirmation email.`
    ...
    activePlan: {
      type: 'string',
      isIn: ['sandbox', 'basic', 'standard', 'pro'],
      defaultsTo: 'sandbox'
    },
  ...
  payoutAccount: async function(userId, walletType) {
    return await PayoutAccount.findOne({ user: userId, type: walletType });
  },

  withActivePlan: async function() {
    return await User.find({ balance: { '>': 0 }, activePlan: { '!=': ['sandbox'] } });
  },

  pricePerMinute: function(user) {
    return pricingPlans.costPerMinute(user.activePlan);
  }
